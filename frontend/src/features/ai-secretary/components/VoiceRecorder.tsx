...
import { formatDurationShort, pickVoiceMime } from '../lib/recorder'
...
type Status = 'idle' | 'requesting' | 'recording' | 'submitting'
...
function checkEnv(): EnvCheck {
  ...
  if (window.isSecureContext === false) {
    return {
      ok: false,
      fail: { code: 'insecure-context', detail: '页面当前不在安全上下文中，浏览器不允许调用麦克风' },
    }
  }
  if (!navigator.mediaDevices || typeof navigator.mediaDevices.getUserMedia !== 'function') {
    return {
      ok: false,
      fail: { code: 'no-mediadevices', detail: '当前浏览器或 WebView 未暴露 mediaDevices 接口' },
    }
  }
  if (typeof window.MediaRecorder === 'undefined') {
    return { ok: false, fail: { code: 'no-recorder', detail: 'MediaRecorder API 不可用' } }
  }
  return { ok: true }
}
...
function explainError(err: unknown): FailInfo {
  ...
  switch (name) {
    ...
    case 'TypeError':
      return { code: 'no-mediadevices', detail: '浏览器拒绝调用麦克风（多见于 HTTP 页面 / 隐私模式）' }
    default:
...
function tipsOf(code: FailInfo['code']): string[] {
  switch (code) {
    ...
    case 'insecure-context':
      return ['在地址栏把 http:// 改成 https://', '或在桌面端用 localhost / 127.0.0.1 访问']
    case 'no-recorder':
    ...
    case 'no-device':
      return ['检查电脑 / 手机是否插入或连接了麦克风', '系统设置中确认默认输入设备可用']
    case 'device-busy':
      return ['关闭正在使用麦克风的其它应用（如腾讯会议、Zoom、语音助手）', '然后回到本页面重试']
    ...
    <div className="flex flex-col gap-3 rounded-md border bg-[var(--color-muted)]/30 p-4">
      <div className="flex items-center justify-center gap-3">
        {!recording ? (
          <Button
            size="lg"
            onClick={handleStart}
            disabled={disabled || fatal || status === 'requesting' || status === 'submitting'}
          >
      ...
      {fail && (
        <div className="rounded-md border border-[var(--color-destructive)]/30 bg-[var(--color-destructive)]/5 p-3 text-xs text-[var(--color-foreground)]">
          ...
          {fail.detail && <p className="mb-2 text-[var(--color-muted-foreground)]">{fail.detail}</p>}
