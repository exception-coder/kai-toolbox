...
export function InitializationProgress({ run }: { run: InitRun }) {
  return <section className="space-y-4" aria-label="初始化进度" aria-live="polite">
    <div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-base font-semibold">{run.mode === 'FULL' ? 'Full Init' : '手动同步'} <span className="ml-2 text-sm font-normal text-[var(--color-muted-foreground)]">{run.state === 'RUNNING' ? '正在执行' : run.state === 'FAILED' ? '执行失败' : '已完成'}</span></h2>
    ...
    <ol className="divide-y divide-[var(--color-border)]">{run.stages.map((stage, index) => <li key={stage.id} className="grid gap-2 py-3 sm:grid-cols-[220px_minmax(0,1fr)]">
      <span className="flex items-center gap-3 text-sm"><span className="w-4 text-xs tabular-nums text-[var(--color-muted-foreground)]">{String(index + 1).padStart(2, '0')}</span>{stage.title}
      ...
      <span className="text-xs leading-relaxed text-[var(--color-muted-foreground)]">{stage.message || '等待执行'}</span>
    </li>)}</ol>
...
export function ProfileOverview({ profile }: { profile: SystemProfile }) {
  return <section className="space-y-6"><div><h2 className="text-base font-semibold">System Profile <span className="ml-2 font-normal text-[var(--color-muted-foreground)]">v{profile.version}</span></h2>
    <p className="mt-2 text-xs text-[var(--color-muted-foreground)]">生成于 {new Date(profile.generatedAt).toLocaleString()} · 五类权威资产入口</p></div>
    <div className="divide-y divide-[var(--color-border)]">{profile.assets.map(asset => <div key={asset.kind} className="flex flex-wrap justify-between gap-3 py-4"><span className="text-sm font-medium">{asset.title}</span><span className="text-xs text-[var(--color-muted-foreground)]">{asset.status === 'READY' ? '已发现证据' : asset.status === 'PARTIAL' ? '部分证据' : '待补齐'} · {asset.sources.length} 个来源</span></div>)}</div>
    {profile.gaps.length > 0 && <div className="border-l-2 border-[var(--color-border)] pl-4"><h3 className="text-sm font-medium">下一步需要补齐</h3><ul className="mt-3 space-y-2 text-sm text-[var(--color-muted-foreground)]">{profile.gaps.map(gap => <li key={gap}>{gap}</li>)}</ul><p className="mt-4 text-xs text-[var(--color-muted-foreground)]">补齐来源后点击“同步画像”重新检查；图谱缺失或过期时执行 Full Init。</p></div>}
  </section>
}
...
export function AssetPanel({ asset }: { asset?: ProfileAsset }) {
  ...
  return <section className="space-y-6"><h2 className="text-base font-semibold">{asset.title}</h2>
    <dl className="space-y-4">{Object.entries(asset.facts).map(([key, value]) => <div key={key} className="grid gap-2 sm:grid-cols-[180px_minmax(0,1fr)]"><dt className="text-xs text-[var(--color-muted-foreground)]">{factLabels[key] ?? key}</dt><dd className="break-words text-sm">{value}</dd></div>)}</dl>
