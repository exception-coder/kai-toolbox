 ...
 * 额度按 {@code toolbox.ai-chat.quota-per-unit} 换算成金额，币种由 {@code currency} 给出。
 *
 ...
 * @param currency        货币符号（如 ¥）；中国服务商为人民币
 * @param usedAmount      已用额度（按 currency 计）
 * @param grantedAmount   授予总额度；无限/为 0 时为 null
 * @param remainingAmount 剩余额度；无限额度时为 null
...
public record UsageInfo(
    ...
    public static UsageInfo unavailable(String error) {
        return new UsageInfo(false, null, null, null, null, null, null, null, error);
    }
}
