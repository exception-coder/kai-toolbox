...
class ReviewIntentServiceTest {
    ...
    void completeRequirementStructureUpgradesUnknownAndExtractsBusinessDraft() {
        ...
        assertThat(result.extractedTitle()).isEqualTo("隐藏工具调用");
        assertThat(result.extractedContent())
                .startsWith("### 理解校对")
                .contains("与面向业务评审的目标不一致", "### 需求说明", "评审页面只展示业务对话",
                        "### 待确认项", "### 验收场景")
                .doesNotContain("需求标题");
    ...
    @Test
    void unstructuredAgentReplyFallsBackToOriginalExpressionWithoutSecondModelRewrite() {
        Fixture fixture = fixture();
        ReviewIntentAssessment before = new ReviewIntentAssessment(
        ...
        assertThat(result.extractedTitle()).isEqualTo("计划评审不要显示工具调用");
        assertThat(result.extractedContent())
                .contains("## 需求说明", "## 待确认项", "## 验收场景")
                .doesNotContain(rawReply, "附件读取失败", "交给开发处理");
        assertThat(result.signals()).contains("回复结构不完整，已按用户原始表达生成待确认草稿");
    }
    ...
    private Fixture fixture() {
        ...
        return new Fixture(classifier, repository,
                new ReviewIntentService(classifier, repository, spaces));
    }

    private record Fixture(ReviewIntentClassifier classifier, ReviewIntentRepository repository,
