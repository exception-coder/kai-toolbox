...
- SpringTransactionAnnotationParser用来解析 `@Transactional` 注解中的属性（比如传播行为、隔离级别等），并将这些属性转化为 Spring 管理事务所需的配置。
...
- 有了代理对象了之后，就需要在事务方法调用的过程中做事务拦截操作。
