    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 p-0 backdrop-blur-sm sm:p-4" onMouseDown={event => event.target === event.currentTarget && !launching && onClose()}>
      <section className="max-h-full w-full overflow-y-auto border border-[var(--color-border)] bg-[var(--color-card)] shadow-2xl sm:max-h-[90vh] sm:max-w-lg sm:rounded-xl">
        <header className="flex items-center justify-between border-b border-[var(--color-border)] px-5 py-4">
          <div className="flex min-w-0 items-center gap-2"><Rocket className="h-4 w-4 shrink-0 text-green-500" /><span className="truncate text-sm font-semibold">{existingDevSessionId ? '继续开发' : '发起 Vibe Coding'} — {title}</span></div>
        ...
        <footer className="flex justify-end gap-3 border-t border-[var(--color-border)] px-5 py-4"><button type="button" disabled={launching} onClick={onClose} className="rounded-lg border border-[var(--color-border)] px-4 py-2 text-sm">取消</button><button type="button" disabled={launching} onClick={() => void launch()} className="flex items-center gap-2 rounded-lg bg-green-600 px-5 py-2 text-sm font-medium text-white disabled:opacity-50">{launching ? <Loader2 className="h-4 w-4 animate-spin" /> : <Rocket className="h-4 w-4" />}{existingDevSessionId ? '打开原会话' : '启动开发会话'}</button></footer>
      </section>
    </div>
  )
}
