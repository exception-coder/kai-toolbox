...
const MANAGED = 'toolbox.claude-chat.business-workspace'
...
export function ProjectDirectorySettings() {
  return <section className="max-w-3xl space-y-8">
    ...
    <DirectoryBlock id={MANAGED} field="root" title="托管业务源码目录" allowDefault />
  </section>
}

function DirectoryBlock({ id, field, title, multiple = false, allowDefault = false }: { id: string; field: string; title: string; multiple?: boolean; allowDefault?: boolean }) {
  ...
  return <div className="space-y-3 border-t border-[var(--color-border)] pt-5">
    ...
    {query.data && <DirectoryEditor key={JSON.stringify(query.data)} block={query.data} field={field} title={title} multiple={multiple} allowDefault={allowDefault} onSaved={() => setSaved(true)} onEdit={() => setSaved(false)} />}
  </div>
}
...
function DirectoryEditor({ block, field, title, multiple, allowDefault, onSaved, onEdit }: { block: ConfigBlockView; field: string; title: string; multiple: boolean; allowDefault: boolean; onSaved: () => void; onEdit: () => void }) {
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      if (!multiple && (values.length > 1 || (!allowDefault && values.length !== 1))) throw new Error('请填写一个完整的本地目录')
  ...
  return <form className="space-y-3" onSubmit={event => { event.preventDefault(); save.mutate() }}>
    <label className="block space-y-2 text-sm"><span className="font-medium">{title}</span>
      <textarea className="w-full rounded-md border border-[var(--color-border)] bg-[var(--color-background)] p-3 text-sm focus-visible:outline-2 focus-visible:outline-[var(--color-ring)]"
        rows={multiple ? 4 : 2} value={draft} disabled={save.isPending} onChange={event => { setDraft(event.target.value); onEdit() }} placeholder={multiple ? '每行一个目录，例如 D:/Projects' : allowDefault ? '留空使用用户目录下的 .kai-toolbox/sources' : '例如 D:/Projects'} />
    </label>
    <p className="text-xs text-[var(--color-muted-foreground)]">{multiple ? '供项目发现、模块工作区与 AI 会话共同使用。' : allowDefault ? '用于 Forge 托管的业务源码。留空恢复默认位置；默认目录尚未创建时不报错，手动指定的失效目录仍会提示。保存不会创建或搬移文件。' : '保留原项目管理的扫描范围与本地文件操作；无需再去配置中心维护。'}</p>
