...
export const getBusinessConsultAgent = () => http<AgentManagementSnapshot>(BASE_PATH)

export function createBusinessConsultCandidate(request: CreateAgentVersionRequest) {
...
export function releaseBusinessConsultCandidate(version: number) {
  return http<AgentManagementSnapshot>(`${BASE_PATH}/versions/${version}/release`, { method: 'POST' })
}

export function rollbackBusinessConsultVersion(version: number) {
  return http<AgentManagementSnapshot>(`${BASE_PATH}/versions/${version}/rollback`, { method: 'POST' })
}
