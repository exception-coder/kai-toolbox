      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)_300px]">
        ...
        <main className="min-h-0 overflow-y-auto px-6 py-6 xl:px-10">
          ...
          {detail && <article className="mx-auto max-w-4xl">
            <div className="mb-7"><span className="rounded-full bg-[var(--color-primary)]/10 px-2.5 py-1 text-xs text-[var(--color-primary)]">{detail.node.nodeType}</span><h2 className="mt-3 text-3xl font-semibold tracking-tight">{detail.node.title}</h2><p className="mt-2 whitespace-pre-line break-words text-[var(--color-muted-foreground)]">{detail.node.summary}</p></div>
