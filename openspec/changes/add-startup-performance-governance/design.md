...
- Portable isolated measurement: `node forge.mjs measure-startup` owns the optional package/temporary-JVM/report lifecycle without connecting to PM2. Reuse configured executable discovery; preserve the report schema, isolated data and optional read-only target. Default port 18090, readiness timeout 120 seconds, optional skip-build/application-jar/output-root. Record failures, reject occupied ports and mismatched run IDs, and stop only owned child handles on success, failure, timeout or interruption. PowerShell becomes a parameter-forwarding compatibility entry. This measures the isolated backend, not the full supervised tool stack.
...
- Node isolated measurement: 16 runtime tests passed, including nine measurement cases with real child processes for readiness, exit, timeout, interrupted build, run-ID mismatch, occupied port, build failure and target HTTP/redirect behavior. Three Task/Compose regressions passed after configuring the existing local Task executable (initial PATH lookup failed). PowerShell forwarding test and all 19 scripts' 5.1/7 parser/encoding checks passed. Actual `node forge.mjs measure-startup --skip-build --application-jar ...` ran the Java protocol fixture, saved COMPLETED and stopped its PID. These are protocol checks, not a full Spring cold-start baseline. Forge returned PASSED/exit 0, no static checkers and nine existing API scenarios. No live restart or full application build was performed.
