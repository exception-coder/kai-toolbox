...
const groups = [
  ...
  ['数据与接口契约', '把代码之外的数据库改动留下来', 'SQL 上下文准备、待执行 SQL 台账、多库目标；接口契约直接写入 OpenSpec；数据库阅读工作区与配置后的测试验证工具。', '入口：待执行 SQL / 数据库 / OpenSpec 变更。登记、实际执行、验证通过是三个不同状态。', 'SessionPendingSqlService / OpenSpec / SessionDatabaseWorkspace'],
  ['评审与业务参与', '把澄清和评审接回研发会话', '分享计划评审、收集文字与附件反馈并带回实现；业务问题回答和风险审批。', '入口：分享计划评审 / 评审。评审可以有独立会话，不等于自动监督另开了一个监督会话。', 'ReviewSpaceService / ReviewIntentService'],
...
export function SessionCapabilityCatalog() {
  return <section className="execution-overview" id="session-capabilities" aria-labelledby="session-capabilities-title">
    ...
    <div className="capability-map" aria-label="Forge 会话能力全景">
      ...
      <div><Database size={24} /><strong>材料与检查</strong><span>SQL 台账 · OpenSpec · 测试环境</span></div>
      <div><Users size={24} /><strong>协作窗口</strong><span>业务评审 · 站点与成果</span></div>
      <div><Bot size={24} /><strong>执行工作台</strong><span>引擎 · 队列 · 权限 · 能力诊断</span></div>
    </div>
    <details className="guide-disclosure" open><summary>三个 Forge 工具：每次调用具体落在哪里？</summary>
      ...
      <p className="guide-note">注入条件：依赖后端连接、会话策略及 forgeSqlRegistration 开关；review-only / disabled 等策略会限制工具注入。不能把三个定义等同于每条会话都能调用。</p>
    </details>
    ...
    <details className="guide-disclosure" open><summary>和直接新建一个 Codex 会话有什么区别？</summary>
      ...
      <div className="guide-table-scroll" tabIndex={0} role="region" aria-label="Forge 与直接使用 Codex 的区别"><table><thead><tr><th>你要完成的事</th><th>直接使用 Codex</th><th>Forge 已实现的接线</th></tr></thead><tbody>
        ...
        <tr><td>交接 SQL、接口契约和业务反馈</td><td>可生成材料或经工具集成保存</td><td>SQL 保存为会话记录；接口契约归入 OpenSpec，并有业务评审入口</td></tr>
      </tbody></table></div>
