  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/20 backdrop-blur-[1px]" onMouseDown={event => event.target === event.currentTarget && onClose()}>
      <aside className="h-full w-full max-w-[520px] overflow-y-auto border-l border-[var(--color-border)] bg-[var(--color-card)] shadow-2xl">
        <div className="sticky top-0 z-10 flex min-h-14 items-center justify-between border-b border-[var(--color-border)] bg-[var(--color-card)]/95 px-5 py-2 backdrop-blur"><span className="min-w-0 truncate pr-3 text-xs font-medium text-[var(--color-muted-foreground)]">需求详情 · {item.id.slice(0, 8).toUpperCase()}</span><button aria-label="关闭需求详情" onClick={onClose} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg hover:bg-[var(--color-muted)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-ring)]"><X className="h-4 w-4" /></button></div>
        <div className="space-y-6 px-5 py-6 sm:space-y-8 sm:p-6">
          <div><div className="flex items-center justify-between gap-3"><DecisionBadge decision={decision} /><span className={`text-[10px] font-medium ${STATUS_META[item.status].cls}`}>{STATUS_META[item.status].label}</span></div><h2 className="mt-5 break-words text-xl font-semibold leading-8">{item.title}</h2><RequirementDescription content={item.description} /></div>
          <dl className="grid grid-cols-1 border-y border-[var(--color-border)] sm:grid-cols-2">
            {[[Building2, '系统 / 模块', `${item.project || '待归属'} / ${item.module || '待归类'}`], [UserRound, '唯一负责人', item.assignee || '待指派'], [CalendarDays, '承诺时间', dateLabel(item.deadline)], [Radio, '数据来源', item.prdSessionId ? `${labels.specification}自动同步` : '统一登记']].map(([Icon, label, value], index) => { const CellIcon = Icon as typeof Building2; return <div key={String(label)} className={`border-b border-[var(--color-border)] py-3 last:border-b-0 sm:border-b-0 ${index % 2 === 0 ? 'sm:pr-4' : 'sm:border-l sm:border-[var(--color-border)] sm:pl-4'} ${index > 1 ? 'sm:border-t' : ''}`}><dt className="flex items-center gap-1.5 text-[11px] text-[var(--color-muted-foreground)]"><CellIcon className="h-3 w-3 shrink-0" />{String(label)}</dt><dd className="mt-1 break-words text-xs font-medium leading-5">{String(value)}</dd></div> })}
          </dl>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
            {!item.prdSessionId && item.status === 'DRAFT' && <button onClick={onClarify} className="flex min-h-11 items-center justify-center gap-2 rounded-lg bg-violet-600 px-4 py-2.5 text-xs font-medium text-white"><Sparkles className="h-3.5 w-3.5" />进入 AI 澄清</button>}
            {item.prdSessionId && prdSession?.status === 'CLARIFYING' && prdSession.questions.length > 0 && <button onClick={onAnswerPrd} className="flex min-h-11 items-center justify-center gap-2 rounded-lg bg-amber-500 px-4 py-2.5 text-xs font-medium text-white"><Sparkles className="h-3.5 w-3.5" />回答澄清问题</button>}
            {item.prdSessionId && prdSession?.status === 'DONE' && <button onClick={onViewPrd} className="flex min-h-11 items-center justify-center gap-2 rounded-lg border border-[var(--color-border)] px-4 py-2.5 text-xs font-medium hover:bg-[var(--color-muted)]"><FileText className="h-3.5 w-3.5" />查看{labels.specification}</button>}
            <button onClick={onDelete} className="flex min-h-11 items-center justify-center gap-2 rounded-lg px-3 py-2.5 text-xs text-rose-500 hover:bg-rose-50 sm:ml-auto dark:hover:bg-rose-950/30"><Trash2 className="h-3.5 w-3.5" />删除</button>
          </div>
        </div>
      </aside>
    </div>
