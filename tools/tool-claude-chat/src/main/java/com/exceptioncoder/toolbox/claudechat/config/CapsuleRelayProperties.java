...
/** 彩虹胶囊宿主身份配置，复用配置中心持久化和刷新。 */
@Getter
@Setter
@Component
@Refreshable(name = "彩虹胶囊接入客户端", group = "Vibe Coding")
@ConfigurationProperties(prefix = CapsuleRelayProperties.PREFIX)
public class CapsuleRelayProperties implements DynamicConfigValidatable {

    /** 与既有部署配置保持相同命名空间。 */
    public static final String PREFIX = "toolbox.claude-chat.session-client.relay";

    /** Relay 入口总开关。 */
    @ConfigDesc("开放业务系统彩虹胶囊接入")
