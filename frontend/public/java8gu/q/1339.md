...
- CompletionStage 是 Java 8 引入的一个接口，位于 java.util.concurrent 包中。
...
- handle 方法接收一个 BiFunction，这个函数有两个参数：计算的结果（如果计算成功完成）和抛出的异常（如果计算失败）。
...
- 那么通过 https://www.yuque.com/hollis666/fsn3og/fg0f0wh41g8eu5ik 我们知道，续期是需要依赖EXPIRATION_RENEWAL_MAP的，如果某个线程不在EXPIRATION_RENEWAL_MAP里面了，就不会再被续期了。
- 所以，如果解锁过程中失败了，redisson也能保证不会再被续期了。
