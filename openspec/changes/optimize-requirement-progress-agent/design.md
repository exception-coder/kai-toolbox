...
6. 需求工程域固定为两个 Agent：`requirement-specification` 负责需求语义与规格产物，`requirement-progress` 负责实现证据与进度结论。执行计划是前者内部的 OpenSpec 同步阶段，不建立第三个 Agent 身份。
7. 首版运行状态不新增平行任务表，而是把 `prd_discovery_run`、PRD 会话执行计划快照和进度运行快照归一化为统一前端节点投影。活动状态禁用重复入口；后端既有幂等登记继续作为最终防线。
...
- [分散运行状态语义不一致] → 使用封闭节点状态映射，保留底层 runId/阶段，不修改既有持久化结构，便于回滚。
