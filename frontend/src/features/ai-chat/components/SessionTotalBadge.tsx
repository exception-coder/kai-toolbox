...
/**
 * 顶部会话累计徽章：累计 token（紫）+ 整会话缓存命中率（青）。
 * 统计本视图所有带指标的助手消息；无可统计（旧消息无指标 / 网关未返回）时不渲染。
 */
export function SessionTotalBadge({ messages, className }: { messages: MessageView[]; className?: string }) {
  const { tokens, durationMs, turns, hitRate } = sessionTotals(messages)
