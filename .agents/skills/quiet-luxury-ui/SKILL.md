...
- **内容最大宽度（Max Width Constraint）**：
  ...
  - 紧凑仪表盘/阅读流：`960px`
  - 工作台、分栏编辑器和数据表格：利用 Shell 分配的可用宽高，不默认套用阅读页最大宽度；仅对需要控制行长的正文局部限宽。
  - 沿 Shell → 页面 → 中间容器 → 面板传递剩余空间并明确滚动归属；禁止固定高度或猜测顶栏像素造成底部闲置。表单控件保持正常密度，不为填满空间而拉伸。
  - 验收高屏、矮屏、窄屏、窗口 resize 和长短内容；实测面板边界及末项、操作栏可达性，不能只看页面宽度或编译结果。
