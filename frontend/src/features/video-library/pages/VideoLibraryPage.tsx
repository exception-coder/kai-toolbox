...
export function VideoLibraryPage() {
  ...
  const handleDelete = async (item: VideoLibraryItem) => {
    const ok = await confirm({
      ...
      description: (
        <div className="space-y-1">
          <div className="break-all font-mono text-xs">{item.name}</div>
          <div className="text-xs tabular-nums">{formatBytes(item.size)}</div>
    ...
    if (!ok) return
    try {
      await deleteFile(item.scanId, item.path)
      // Advance selection BEFORE removing from cache so we read the right neighbour.
