...
/** 项目内只读探索入口与带证据的领域草稿。 */
export function BusinessKnowledgePanel({ projectId }: { projectId: string }) {
  ...
  return <section className="space-y-8">
    <header className="max-w-3xl space-y-2"><p className="text-xs text-[var(--color-muted-foreground)]">{data?.stale ? '证据过期' : snapshot ? '有候选 · 待核实' : '未探索'}</p><h2 className="text-lg font-semibold">从代码理解业务域</h2>
