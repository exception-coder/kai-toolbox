...
| 版本 | 日期 | 修改人 | 变更内容摘要 |
|---|---|---|---|
| v18 | 2026-08-23 | Codex | 规划评估增加“首版上测试环境”时间：AI 选择最小业务闭环，服务端校验能力引用并确定性汇总单主开发线工作日，同时展示首版范围与暂缓项 |
| v17 | 2026-08-23 | Codex | 证据消费一致性加固：附件转为有界业务线索与技术坐标，多项目轨迹按当前实现/遗留来源归纳，规划 Prompt 不再传整份轨迹，并以确定性门禁拒绝“已有命中却声称全部缺失” |
| v16 | 2026-08-23 | Codex | 接入 planning-evidence-discovery 动态编排：确定性解析主项目与重构/迁移/依赖/集成范围，按关系执行六类证据查询，持久化 trace v2 并以三轮服务端门禁约束规格、价值判定和工时规划 |
| v15 | 2026-08-23 | Codex | 价值判定升级为持久化后台任务，支持幂等登记、刷新追踪、失败留痕和应用重启恢复 |
| v14 | 2026-08-23 | Codex | 价值判定升级为规划评估上游快照，分析完成自动刷新关联工时并统一过期口径 |
...
- **证据编排结论**：提示词不再固定预拼来源。平台先解析受控项目范围，Agent 根据 `planning-evidence-discovery` 形成六类查询计划，平台执行并登记真实调用，服务端按关系规则补齐必查项并裁决完成性。
...
| 范围先行 | 第一个证据动作必须解析主项目及 `REFACTORS / MIGRATES_FROM / DEPENDS_ON / INTEGRATES_WITH` 关系；模型不得猜测项目路径、知识键或依赖关系。 |
| 动态计划 | Agent 只负责根据需求与受控范围决定查询矩阵和原因；平台校验项目坐标、执行工具并补齐关系规则规定的必查项。 |
| 轨迹同源 | 初始化规格、价值判定和工时规划引用同一个 `planning-evidence-trace-v2.traceId`；重新探索生成新轨迹，历史空轨迹不回填伪造。 |
| 关系分栏 | 当前实现、遗留重构来源、迁移来源、普通依赖和集成方证据必须分栏；旧项目证据只证明迁移或复用参考，不得写成新项目已实现事实。 |
| 查询完成性 | `NO_HIT / SOURCE_MISSING / EXECUTION_ERROR` 表示已执行但无可用证据，不等同于漏查；只有完全缺少必查 ledger entry 才进入下一轮，最多三轮。 |
...
| 价值判定后台化 | 单条价值判定先持久化 `req_pool_insight_run` 再提交任务执行器；HTTP 只负责登记并立即返回，页面刷新、关闭或网络中断不取消运行，应用启动后恢复 `RUNNING` 任务。 |
| 判定幂等与冻结 | 同一需求同一时刻只允许一条 `RUNNING` 判定；运行冻结标题、描述、项目、模块、来源指纹和执行引擎，重复点击返回现有运行，不重复调用模型。 |
| 判定完成联动 | 价值判定通过代码校验并写入不可变洞察历史后，运行才标记 `COMPLETED`，随后异步登记引用该洞察的规划工时；判定失败保留旧洞察和旧规划并记录可恢复错误。 |
| 项目理解复用 | 价值判定后台任务通过公共只读端口主动查询业务知识、Graphify、关键 DDL 和路由映射；用户正文与附件先压缩为最多 2400 字的“需求摘要、附件业务线索、技术坐标”，禁止把附件全文送入图谱查询。命中状态、目标与有界摘要随洞察冻结。规划优先复用该轨迹，`HIT` 来源不得再次笼统表述为缺失；查询能力不可用时才降级复用最近规划轨迹。 |
| 评估输入冻结 | 规划评估只使用本次确认的初始化规格全文和会话元数据，保存输入 SHA-256；不读取生成后可能变化的核心规格，保证同一输入可复跑对比。 |
| 首版测试环境时间 | 规划评估必须额外选择可独立演示、可收集反馈的最小业务闭环；模型只返回首版范围、纳入的能力 ID 与暂缓项，服务端校验能力引用并从已归一化工作包确定性汇总时间。缺少开工日和团队并行度时只展示单主开发线工作日区间，不伪造日历日期。 |
| 业务能力拆分 | 拆分项按业务领域、业务结果和独立验收边界组织，不按 Controller、接口或表等纯技术任务拆分。 |
| 评估准则单一来源 | 规划工时使用版本化标准 `initial-spec-planning-v4`；工作包口径、上下限、共享成本去重、置信度缓冲、人日换算和首版测试环境工作日由代码固化，Prompt 只引用该标准生成结构化建议。 |
| 规划证据可追溯 | 确认初始化规格时冻结项目、角色、关系、模块、查询词以及六类来源的调用目标、调用状态和有界摘要；评估页常驻显示“当前实现 / 遗留来源 / 依赖项目”的命中摘要，逐调用轨迹继续折叠，用于区分“当前尚未实现”“关联项目已有迁移依据”“数据源缺失”“已查询未命中”和“未记录”。 |
| 证据结论一致性 | 规划 Prompt 只接收按项目角色归纳的有限摘要和命中摘录，不传整份轨迹 JSON；模型输出若把关联项目已命中的业务知识、Graphify 或 DDL 概括为全局缺失，确定性校验必须拒绝并进入最多三次自动纠正。 |
...
- **跨项目证据**：当前实现项目无图谱但遗留项目已命中时，页面和规划结果必须同时表达“当前缺口”和“遗留迁移依据”，不得显示为全局无法分析。
...
- 领导视图在完整投入之前优先展示“首版上测试环境”工作日区间、可验证业务闭环和暂缓项；该时间不是完整交付承诺，也不是把总工时按比例缩小。
- 首版必须至少纳入一个领域能力，且只能引用本次规划已存在的能力 ID；首版工作日由服务端汇总所选能力归一化后的工时，AI 不得直接输出权威总量。
- 规划准则升级为 `initial-spec-planning-v4`，按 AI Code Agent 主导实现、人负责审查验收的有效工时估算，并把规划时实际使用的证据调用轨迹固化到评估快照。
- 规划 Prompt 升级为 `initial-spec-planning-prompt-v5`：增加最小可验证闭环选择，继续消费角色级证据摘要与有界命中摘录，并在落库前校验结论是否与轨迹冲突。
...
- 价值判定是规划评估的上游事实快照；规划任务登记时冻结判定 ID、判定指纹和结构化结论，禁止执行阶段动态读取另一版本。
- 规划 Prompt 复用价值判定中的业务目标、优先级、ROI、影响范围、推荐方向和风险理由；价值判定的 `estimatedHours` 只用于结果偏差校验，不作为正式人日输入或锚点。
- 规划幂等指纹同时包含初始化规格与上游判定指纹。重新分析产生新判定后，旧规划立即标记为待同步，并自动登记新的后台规划任务。
- 历史需求尚无价值判定时允许仅按初始化规格完成规划；首次生成判定后自动补齐关联，不阻断已有规格交付。
- 用户侧统一为“需求评估”工作流，进度按“价值判定、功能与工时规划”展示；内部仍保留两类可独立审计的产物。
- 规划运行持久化为后台任务；刷新或离开页面不影响执行，应用启动后会恢复账本中仍为 `RUNNING` 的任务。
