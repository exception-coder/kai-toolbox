...
- [x] 4.4 禁止能力探测及执行失败清空原生关联，补回归并恢复指定会话历史。
...
关联保护修复验证：TypeScript 编译及 64 项 Codex 测试通过；Forge 门禁 exit 0、status PASSED，9 项 API 校验通过，静态 checker 未执行。指定会话已备份并恢复原生关联，最近及更早分页各返回 30 条；受控重载后稳定观察至少 60 秒。未执行付费模型对话或浏览器视觉验收，4.3 保持未完成。
