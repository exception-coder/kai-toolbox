...
export const stageDescriptions: Record<string, { label: string; check: string; output: string; purpose: string }> = {
  ...
  semantic: { label: '业务语义分析', check: '代码探索领域快照及源码、图谱新鲜度；OpenSpec 仅作补充规格。', output: '在业务域页选择 Codex 或 Claude Code，从 Graphify 和源码归纳领域；初始化收录已有快照，不自动调用模型。', purpose: '沉淀带代码证据、置信度与覆盖缺口的领域草稿，供任务复用。' },
