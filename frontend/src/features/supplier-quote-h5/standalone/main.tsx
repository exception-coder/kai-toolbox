...
async function start() {
  ...
  try {
    ...
    const gateway =
      config.mode === "http"
        ? createHttpSupplierQuoteGateway({
            apiBaseUrl: config.apiBaseUrl,
            marketQuoteApiBaseUrl: config.marketQuoteApiBaseUrl,
          })
...
async function loadRuntimeConfig(): Promise<RuntimeConfig> {
  ...
  if (!response.ok)
    throw new Error("缺少 runtime-config.json，无法确定报价服务地址");
  const config = (await response.json()) as Partial<RuntimeConfig>;
  if (config.mode !== "mock" && config.mode !== "http")
    throw new Error("runtime-config.json.mode 必须是 mock 或 http");
  if (config.mode === "http" && !config.apiBaseUrl?.trim())
    throw new Error("HTTP 模式必须配置 apiBaseUrl");
  return {
    mode: config.mode,
    apiBaseUrl: config.apiBaseUrl?.trim() ?? "",
    marketQuoteApiBaseUrl: config.marketQuoteApiBaseUrl?.trim(),
    brandName: config.brandName?.trim() || "织联协同",
  };
}
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-50 p-6 text-slate-900">
      <section className="w-full max-w-lg rounded-3xl border border-red-200 bg-white p-7 shadow-xl">
        ...
        <p className="mt-5 rounded-xl bg-slate-100 p-3 font-mono text-xs text-slate-600">
          请检查站点根目录的 runtime-config.json
        </p>
      </section>
    </main>
  );
