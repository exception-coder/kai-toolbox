...
function removeLegacyPrefix(raw: string, prefix: string): string | undefined {
...
export function normalizeUserMessageForDisplay(raw: string): string {
  ...
  const consultText = removeLegacyPrefix(raw, LEGACY_CONSULT_READONLY_PREFIX)
  if (consultText !== undefined) return consultText
  const forgeText = removeLegacyPrefix(raw, LEGACY_FORGE_PENDING_SQL_PREFIX)
