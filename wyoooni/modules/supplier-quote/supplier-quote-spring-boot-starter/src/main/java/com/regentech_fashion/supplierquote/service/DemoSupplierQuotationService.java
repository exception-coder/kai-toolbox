...
import com.regentech_fashion.supplierquote.infrastructure.local.LocalSupplierQuotePersistence;
...
public class DemoSupplierQuotationService implements SupplierQuotationUseCase {
    private static final String DEMO_TICKET = "demo-quote";
    private static final Set<String> ITEM_IDS = Set.of("item_01", "item_02");
    private final LocalSupplierQuotePersistence repository;
    private final ObjectMapper objectMapper;

    public DemoSupplierQuotationService(LocalSupplierQuotePersistence repository, ObjectMapper objectMapper) {
    ...
    public BindingView requireBinding(String subjectHash) {
        return repository.findBindingBySubject(subjectHash).orElseThrow(() ->
                new SupplierQuoteApiException(HttpStatus.PRECONDITION_REQUIRED, "BUSINESS_ACCOUNT_BINDING_REQUIRED",
                        "请先使用公司业务账号完成首次关联"));
    }
    ...
    public QuotationAccess access(String ticket, BindingView binding) {
        ...
        var submission = repository.findSubmission(ticket, binding.accountId());
        var draft = repository.findDraft(ticket, binding.accountId());
    ...
    public DraftReceipt saveDraft(String ticket, BindingView binding, QuotationDraftRequest request) {
        return repository.inTransaction(() -> saveDraftInTransaction(ticket, binding, request));
    }

    private DraftReceipt saveDraftInTransaction(String ticket, BindingView binding,
                                                QuotationDraftRequest request) {
        requireDemoTicket(ticket);
        validate(request);
        if (repository.findSubmission(ticket, binding.accountId()).isPresent()) {
            throw new SupplierQuoteApiException(HttpStatus.CONFLICT, "QUOTATION_ALREADY_SUBMITTED", "报价已提交，不能再保存草稿");
        }
        long now = System.currentTimeMillis();
        var saved = repository.saveDraft(ticket, binding.accountId(), write(request), request.draftVersion(), now);
        if (saved == null) {
    ...
    public SubmissionReceipt submit(String ticket, BindingView binding, String idempotencyKey,
                                    QuotationDraftRequest request) {
        return repository.inTransaction(() -> submitInTransaction(ticket, binding, idempotencyKey, request));
    }

    private SubmissionReceipt submitInTransaction(String ticket, BindingView binding, String idempotencyKey,
                                                  QuotationDraftRequest request) {
        requireDemoTicket(ticket);
        ...
        validate(request);
        var existing = repository.findSubmission(ticket, binding.accountId());
        ...
        var savedDraft = repository.saveDraft(ticket, binding.accountId(), write(request), request.draftVersion(),
        ...
        repository.insertSubmission(ticket, binding.accountId(), idempotencyKey, submissionId, write(request), now);
