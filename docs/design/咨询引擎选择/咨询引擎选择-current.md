...
- 业务咨询由管理员选择并保存统一的 Codex 授权目录；所有用户的新咨询使用该服务端配置。
...
```mermaid
flowchart LR
    UI["管理员保存业务默认模型和 Auth"] --> POLICY["咨询策略单例"]
    POLICY --> WS["新咨询写入 Auth 快照"]
    WS --> DB["会话元数据持久化"]
    WS --> SC["Node sidecar"]
...
- 业务咨询服务端不得硬编码某个账号目录，也不得信任客户端逐次覆盖；新咨询从管理员单例策略读取 `codexHome`，随后以会话持久化值作为 attach、resume 和自愈重连的唯一事实源。
- Codex 客户端缓存键包含授权目录和速度档位，防止不同账号复用同一客户端实例。
- 空授权目录表示沿用默认 `%USERPROFILE%\.codex`。
- 业务咨询仅允许管理员编辑 Auth 目录，普通用户只读看到业务默认；管理员选择未保存时不得发起咨询。
- Vibe Coding 仍使用自己的最近授权目录偏好；业务咨询不复用浏览器偏好。第三方网关仍不使用本地授权目录。
...
- 管理员只能保存服务端发现的目录；旧策略尚未配置 Auth 时阻止新咨询并提示完成配置。
- 请求中伪造或沿用旧版客户端传入的 `codexHome` 不得覆盖业务默认。
- 首次提问、后续追问、页面重连和后端重启恢复使用同一条 `codexHome`，服务端不在恢复阶段覆盖账号目录。
...
- 发起咨询前必须选择业务系统，并填写问题标题；会话保存系统、模块、登录用户、问题标题和首个明确问题。
...
- 会话问题标题由用户填写，发送首条消息前必须通过非空校验。前端取用户本机当前时刻对应的 UTC 日期，格式化为 `yyMMdd`，并按 `yyMMdd-用户标题` 生成最终标题。
- 最终标题不超过 40 字：日期前缀与连接符占 7 字，用户填写部分最多 33 字；后端在创建会话时校验并一次性保存，后续追问和轮次同步不得改写标题。
- 历史咨询列表允许有权访问该会话的用户重命名标题正文。重命名保留原有 UTC 日期前缀；存量标题没有日期前缀时，按会话创建时间换算 UTC 后补齐。
- 重命名只修改 `question_title`，不改变首问、咨询轮次、归档状态或创建时间。
...
flowchart TD
    ...
    F -->|"否"| T{"标题是否已填写?"}
    T -->|"否"| B["阻止发送并提示填写标题"]
    T -->|"是"| S["添加UTC日期前缀并固定标题"]
    H["历史咨询重命名"] --> P["保留或补齐UTC日期前缀"]
    P --> U["更新问题标题"]
...
- 用户点击“分析链路”前必须显式选择 Claude Code 或 Codex；初始选择为 Claude Code，保持现有默认行为可见。
- 前端将选择的 `engine` 与当前可见系统清单一起提交，后端只接受 `claude`、`codex`，空值兼容归一为 `claude`。
- 分析成功且返回边时，替换已持久化链路并展示成功提示与边数量。
- 分析成功但没有可信边时，展示“未发现链路”提示，并保留上一次已持久化链路，避免一次空结果清空全部路线。
- 请求失败时展示后端错误，不修改当前链路。

```mermaid
flowchart TD
    S["选择Claude Code或Codex"] --> A["提交系统清单和engine"]
    A --> R{"分析结果"}
    R -->|"存在可信边"| P["替换持久化链路并显示数量"]
    R -->|"空结果"| K["保留旧链路并提示未发现"]
    R -->|"失败"| E["保留旧链路并显示错误"]
```
