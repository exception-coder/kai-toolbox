...
import { ArrowDownToLine, ArrowUpFromLine, Check, Copy, Download, Minimize2, Sparkles } from 'lucide-react'
...
import { xmlEscape, xmlFormat, xmlMinify, xmlUnescape } from '../lib/xml'
...
export function XmlPanel() {
  ...
  const split = useSplitRatio('formatter.xml.splitRatio')

  const indentVal: number | '\t' = indent === 'tab' ? '\t' : Number.parseInt(indent, 10)

  type Op = 'format' | 'minify' | 'escape' | 'unescape'
  const dispatch = useCallback(
    (op: Op) => {
      setError(null)
      const input = inputRef.current?.getValue() ?? ''
      let res
      if (op === 'format') res = xmlFormat(input, { indent: indentVal })
      else if (op === 'minify') res = xmlMinify(input)
      else if (op === 'escape') res = { ok: true as const, output: xmlEscape(input) }
      else res = { ok: true as const, output: xmlUnescape(input) }
  ...
  const onDownload = useCallback(() => {
    const v = outputRef.current?.getValue() ?? ''
    if (!v) return
    const blob = new Blob([v], { type: 'application/xml' })
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-end gap-4">
        ...
        <div className="flex flex-wrap gap-2 self-end">
          ...
          <Button onClick={() => dispatch('escape')} size="sm" variant="outline">
            <ArrowUpFromLine /> 转义
          </Button>
          <Button onClick={() => dispatch('unescape')} size="sm" variant="outline">
            <ArrowDownToLine /> 反转义
          </Button>
        </div>
      </div>
