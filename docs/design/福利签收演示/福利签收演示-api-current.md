...
实现选择**演示会话不持久化**：demo 会话只活在内存（`SessionCtx.demo=true`），不写 `claude_chat_session`，
故天然不进正式会话列表，也**无需给 claude_chat_session 加列/ 迁移**。每会话的 demo 库按 `welfare-sign-schema.sql`
建在 `${data-dir}/welfare-demo/<sandboxId>.db`，断连/超时即删。
