...
- 官方 Codex 咨询支持输入区语音对话、静音和挂断，双方转写显示在原有消息中。

### 语音对话

在 HTTPS 或 localhost 页面打开胶囊，等待会话连接后点击“语音对话”，首次使用需允许麦克风。语音沿用当前项目与页面上下文，仍为只读咨询；提出开发需求继续使用现有需求交接流程。Claude 和第三方网关暂不支持此入口的原生语音。

关闭胶囊、切换页面、断线或离开页面会释放收音和播放。刷新后手动点击语音入口恢复；同一语音线程以最后主动连接的设备接管，旧设备不会自动抢回。原本由文字启动且仍在执行的任务没有可恢复的语音线程时，会提示稍后重试，不中断该任务。

音频协商不进入日志或待发送队列。实时转写保留在当前页面消息列表；刷新或换设备后不恢复尚未归档的实时转写。自定义 Transport 可实现 `startVoice(snapshot)`、`stopVoice()`、`muteVoice()` 并发布 `AssistantWidgetState.voice`；未实现时不显示语音控件。
