...
| 27 | 异常处理 | AI 重新分析入口未携带或继承用户选择的引擎，且前端只清理加载状态、不呈现上游额度或鉴权失败 | Agent 重试必须显式传递受控执行配置；默认继承最近一次成功配置，无历史时使用产品默认值，并把失败转化为保留上下文的可恢复状态 | ReqPoolSections.tsx、useReqpoolItemCommands.ts、ReqInsightApplicationService.java | 2026-08-23 | 1 |
| 28 | 架构约束 | 跨模块 Hook 经聚合公共入口消费，但顶层 Provider 直接引用内部 Context，热更新重新求值后可能形成两个 Context 身份并使模块崩溃 | Provider 与跨模块 Hook 必须统一走稳定的运行时公共入口；长生命周期 Context 需在同一页面保持单例，并用模块重新求值回归测试覆盖 | ChatRuntimeContext.tsx、public-api/runtime.ts、ReqPoolPage.tsx | 2026-08-23 | 1 |
| 29 | 分层依赖 | 需求价值判定与规划工时评估各自从原始输入独立推理，规划层没有消费已生成的业务价值、影响范围与风险结论，形成重复分析和两套不一致口径 | 价值判定作为带版本和输入指纹的上游事实快照；规划评估显式引用并复用其业务结论，正式工时由统一规划准则计算，粗估仅用于偏差校验 | ReqInsightApplicationService.java、ReqPlanningAssessmentService.java、ReqPlanningCommand.java | 2026-08-23 | 1 |
| 30 | 架构约束 | 将确定性的 Yoooni DDL 抽取、快照写入与定时登记实现放入 Agent 编排插件，导致执行能力依赖自然语言入口且缺少统一运行状态 | 固定执行器、手动/定时触发、运行记录和失败诊断归属 Forge `tool-knowledge-graph`；插件 Skill 仅调用 Forge 稳定入口并解释结果，凭据不进入仓库、插件或任务参数 | yoooni-ddl-sync Skill、sync-yoooni-ddl.ps1、register-ddl-sync-task.ps1、tool-knowledge-graph | 2026-08-23 | 1 |
| 31 | 架构约束 | 把 Forge 中名为 `yoooni-one` 的已登记数据源误解为 Yoooni One 进程内连接池，额外新增跨项目 HTTP 写入服务 | 反馈识别与持久化全部留在 Forge，通过 `tool-ops` 中 `yoooni-one` 数据源的既有连接池直接写入公网 MySQL | YoooniOneAssistantFeedbackStoreAdapter.java、assistant-feedback | 2026-08-25 | 1 |
| 32 | 数据建模 | 将用户原描述、AI 优化稿、用户改写压缩为 `source_content` 加混合语义的 `feedback_content`，并依赖 revision 反推主表事实 | 主表显式保存 `source_content`、`ai_optimized_content`、`user_rewritten_content`；有效正文只派生，revision 仅用于不可变审计 | assistant-feedback-schema.sql、AssistantFeedbackStorePort.java、OpsAssistantFeedbackStoreAdapter.java | 2026-08-25 | 1 |
