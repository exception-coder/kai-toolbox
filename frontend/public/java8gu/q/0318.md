...
- 对于消息的重复消费问题，比较常见的解决方式，就是通过**消息中定义的一个幂等号，来做防重判断**。
- 而对于重复下单的场景，这个幂等号应该怎么产生呢？
...
- 这是一个token校验的简单实现，但是这个逻辑存在一个问题，那就是如果高并发场景，可能会导致多个线程同时，导致token同时校验通过。
- 如使用Redis的事务：
...
- 但是，这个方案还存在一个问题，那就是如果用户提前先通过token获取的接口拿到一批token，然后再风控下单，也能绕过这个防重校验了。
