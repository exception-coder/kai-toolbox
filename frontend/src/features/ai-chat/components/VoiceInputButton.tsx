 ...
 * 录音中展开成「✕ 取消 · 跳动点 + 秒数 · ■ 停止并转写」胶囊；识别中显示「识别中…」；
 * 空闲态为麦克风按钮。浏览器不支持或 ASR 未就绪时禁用并提示。
 */
export function VoiceInputButton({ onText, disabled }: { onText: (t: string) => void; disabled?: boolean }) {
  ...
  const title = !rec.supported
    ? '当前浏览器不支持录音'
    : available === false
      ? 'ASR（faster-whisper）未启动，语音输入不可用'
      : err
        ? err
        : '语音输入'

  return (
    <Button type="button" variant="ghost" size="icon" onClick={start} disabled={disabled || off} title={title} aria-label={title}>
