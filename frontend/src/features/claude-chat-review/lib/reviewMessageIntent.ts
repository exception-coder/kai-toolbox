...
export function projectReviewTurns(
  ...
  const finish = (completed: boolean) => {
    ...
    const intent = parsed.intent === 'UNCLASSIFIED' && !completed ? 'PENDING' : parsed.intent
    turns.push({ userItem, assistantItems, intent, assistantText: parsed.text, completed })
    userItem = null
    assistantItems = []
  }
...
export function requirementsFromTurns(turns: ReviewTurn[], reviewCreatedAt: number): ReviewRequirement[] {
  ...
  for (const turn of turns) {
    if (turn.intent !== 'REQUIREMENT') continue
    ...
    const content = (titleMatch ? assistantText.replace(titleMatch[0], '').trim() : assistantText)
