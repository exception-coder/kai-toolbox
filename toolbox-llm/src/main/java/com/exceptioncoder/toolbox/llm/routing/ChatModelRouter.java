...
public class ChatModelRouter {
    ...
    private final Map<String, RoutingChatModel> tierModels = new ConcurrentHashMap<>();
    private final RoutingChatModel all;

    public ChatModelRouter(Map<String, List<ModelEndpoint>> byTier, List<ModelEndpoint> allEndpoints) {
        byTier.forEach((tier, eps) -> tierModels.put(tier, new RoutingChatModel(tier, eps)));
        this.all = new RoutingChatModel("*", allEndpoints);
    }

    /** 指定档位的路由模型；该档位无配置时回退到全体模型池。 */
    public ChatModel forTier(String tier) {
        RoutingChatModel m = tierModels.get(tier);
        return m != null ? m : all;
    }
