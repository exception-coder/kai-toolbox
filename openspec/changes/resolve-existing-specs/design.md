...
Writer recovery does not use elapsed time or assume that a missing host session is dead. While binding a new execution under the same atomic state lock, Forge may reclaim the previous writer only when its record matches the writer pointer and assigned branch, HEAD is a committed descendant of the baseline, required verification is current and passing, and the implementation, design and associated Change scope are clean. The previous execution record remains with an `AUTO_RECLAIMED` audit entry; its session pointer and writer pointer are removed before the new writer is saved. Any unreadable, inconsistent, unverified, uncommitted or dirty state fails closed as `WORKSPACE_BUSY`.
