...
const DRAFTS_KEY = 'kai-toolbox:claude-chat-stable:drafts'
/** 无会话（新建面板等）时草稿的占位键。 */
...
const SPLIT_STATE_KEY = 'kai-toolbox:claude-chat-stable:split-state'
...
export function ChatPage() {
  ...
  if (!seedReadRef.current) {
    ...
    try {
      const seed = sessionStorage.getItem('kai-toolbox:claude-chat-stable:aggregation-draft')
      if (seed) { sessionStorage.removeItem('kai-toolbox:claude-chat-stable:aggregation-draft'); seedRef.current = seed }
    } catch { /* 忽略隐私模式异常 */ }
  }
  ...
  useEffect(() => {
    ...
    try { raw = sessionStorage.getItem('kai-toolbox:claude-chat-stable:erp-dev-launch') } catch { /* ignore */ }
    if (!raw) return
    erpLaunchedRef.current = true
    try { sessionStorage.removeItem('kai-toolbox:claude-chat-stable:erp-dev-launch') } catch { /* ignore */ }
    try {
  ...
  useEffect(() => {
    let raw: string | null = null
    try { raw = sessionStorage.getItem('kai-toolbox:claude-chat-stable:open-panel') } catch { /* ignore */ }
    if (!raw) return
    try { sessionStorage.removeItem('kai-toolbox:claude-chat-stable:open-panel') } catch { /* ignore */ }
