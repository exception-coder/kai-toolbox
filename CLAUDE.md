...
本项目是 AI 原生项目。执行或间接触发任何服务重启前，必须说明目标、原因和影响，等待用户对本次重启明确确认；不得以开发、修复或交付验收为由自行重启。统一授权边界见 [AGENTS.md 的重启确认规则](AGENTS.md#服务重启必须经用户明确确认)，其优先于下述验收与自动提交要求。

编译、启动、重启或源码自动更新后，必须执行 [AGENTS.md 的运行验收与自动修复闭环](AGENTS.md#自动编译与重启必须完成运行验收)：不能凭命令成功或端口开放结束任务；失败主动修复并重验，运行验证及至少 60 秒稳定观察通过后才能交付。该规则为唯一权威，不在本文件复制维护。
