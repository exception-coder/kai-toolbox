...
import { ArrowLeftRight, Check, Copy, Download, Minimize2, Sparkles } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Segmented } from '@/components/ui/segmented'
import { cn } from '@/lib/utils'
import { CodeEditor, type CodeEditorRef, type EditorLanguage } from './CodeEditor'
import { SplitPane } from './SplitPane'
import { useSplitRatio } from '../lib/useSplitRatio'
import { jsonToYaml, yamlFormat, yamlMinify, yamlToJson } from '../lib/yaml'
...
export function YamlPanel() {
  ...
  /** 互转时输入侧的语言会反一下，保持高亮匹配实际内容 */
  const [inputLang, setInputLang] = useState<EditorLanguage>('yaml')
  const [outputLang, setOutputLang] = useState<EditorLanguage>('yaml')
  const [outputExt, setOutputExt] = useState<'yaml' | 'json'>('yaml')
  const inputRef = useRef<CodeEditorRef>(null)
  const outputRef = useRef<CodeEditorRef>(null)
  const split = useSplitRatio('formatter.yaml.splitRatio')
  const indentVal = Number.parseInt(indent, 10)

  type Op = 'format' | 'minify' | 'toJson' | 'fromJson'
  const dispatch = useCallback(
    (op: Op) => {
      setError(null)
      const input = inputRef.current?.getValue() ?? ''
      let res
      if (op === 'format') res = yamlFormat(input, { indent: indentVal })
      else if (op === 'minify') res = yamlMinify(input)
      else if (op === 'toJson') res = yamlToJson(input, { indent: indentVal })
      else res = jsonToYaml(input, { indent: indentVal })
      ...
      if (res.ok) {
        ...
        // 互转时同步把输入侧高亮切到对应语言
        if (op === 'fromJson') setInputLang('json')
        else if (op === 'toJson') setInputLang('yaml')
      } else {
  ...
  const onDownload = useCallback(() => {
    const v = outputRef.current?.getValue() ?? ''
    if (!v) return
    const mime = outputExt === 'json' ? 'application/json' : 'text/yaml'
    const blob = new Blob([v], { type: mime })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `formatted-${Date.now()}.${outputExt}`
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-end gap-4">
        ...
        <div className="flex flex-wrap gap-2 self-end">
          ...
          <Button onClick={() => dispatch('toJson')} size="sm" variant="outline">
            <ArrowLeftRight /> YAML → JSON
          </Button>
          <Button onClick={() => dispatch('fromJson')} size="sm" variant="outline">
            <ArrowLeftRight /> JSON → YAML
          </Button>
        </div>
      </div>

      <p className="text-[11px] text-[var(--color-muted-foreground)]">
        提示：互转会丢失 YAML 的注释与锚点引用（!&amp;ref）；仅保留数据结构。
      </p>
