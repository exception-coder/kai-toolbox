...
import { folders, hash, locked, readJson, readText, safePath, saveJson, statePath } from './storage.js'
import { indexSpecs } from './indexer.js'
import { graphEvidence, retrieve } from './retriever.js'
import { checkReadiness } from './service.js'
import { assessExecutionSchema, discoverExecutionSchema, executionCheckSchema, executionContextSchema, executionPolicy, type Assessment } from './executionContracts.js'
...
function context(project: string) {
  ...
  requireCondition(branch, 'BRANCH_REQUIRED', '先由宿主分配工作分支；不在 detached HEAD 上自动创建分支')
  return { root, branch }
}
...
type Discovery = ReturnType<typeof buildDiscovery>
function buildDiscovery(input: ReturnType<typeof discoverExecutionSchema.parse>, root: string, branch: string) {
...
export function discoverExecution(raw: unknown) {
  const input = discoverExecutionSchema.parse(raw); const { root, branch } = context(input.project)
...
export function assessExecution(raw: unknown) {
  const input = assessExecutionSchema.parse(raw); const { root, branch } = context(input.project)
...
export function checkExecution(raw: unknown) {
  const input = executionCheckSchema.parse(raw); const { root, branch } = context(input.project)
