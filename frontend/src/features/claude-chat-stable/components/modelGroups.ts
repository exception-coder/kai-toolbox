...
// 按模型 id 关键词推断所属平台。顺序即展示顺序：Claude 在前（Claude 引擎主场），其余常见家族其次，'other' 兜底。
const RULES: { key: string; label: string; re: RegExp }[] = [
  { key: 'anthropic', label: 'Claude · Anthropic', re: /claude|anthropic|sonnet|opus|haiku/ },
  { key: 'openai', label: 'OpenAI · GPT', re: /\bgpt|^o[1-9]|chatgpt|openai|davinci|dall-?e|whisper/ },
...
export function groupModels(models: ModelInfo[]): ModelGroup[] {
  ...
  for (const m of models) {
    const k = modelPlatform(m.value).key
    const arr = map.get(k)
  ...
  return ORDER.filter(k => map.has(k)).map(k => ({
    key: k,
    label: LABELS[k] ?? k,
    models: map.get(k)!.slice().sort((a, b) => a.value.localeCompare(b.value)),
  }))
}
